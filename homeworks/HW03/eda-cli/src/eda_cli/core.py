...
def missing_table(df: pd.DataFrame) -> pd.DataFrame:
    """
    Таблица пропусков по колонкам: count/share, а также дубликатов
    """
    if df.empty:
...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # Новая эвристика качества - дублирование строк (количество)
    flags["duplicated_rows"] = missing_df["duplicate"].sum()
    
    # Новая эвристика - столбец с одинаковыми значениями
    ...
    score -= missing_df["duplicate"].sum() * 0.1  # чем больше дублирований, тем хуже
    if flags["has_constant_columns"]:  # плохо, если есть столбец с одинаковыми значениями
        score -= 0.1
