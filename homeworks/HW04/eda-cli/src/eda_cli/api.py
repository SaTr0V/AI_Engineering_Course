...
# ---------- /quality-flags-from-csv: получение эвристик из CSV ----------


@app.post(
    "/quality-flags-from-csv",
    tags=["quality"],
    summary="Получить флаги качества данных по CSV-файлу",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict:
    """
    Принимает CSV-файл, применяет EDA-логику и возвращает полный словарь флагов качества
    в чистом JSON-формате (включая quality_score, duplicated_rows, has_constant_columns и т.д.).
    """
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        # content_type от браузера может быть разным, поэтому проверка мягкая
        # но для демонстрации оставим простую ветку 400
...
# ---------- /head: просмотр первых n строк датасета ----------


@app.post(
    "/head",
    tags=["dataset"],
    summary="Просмотр первых N строк CSV-файла"
)
async def head_csv(
    ...
    """
    Возвращает первые n строк CSV как список словарей.
    """
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
    ...
    try:
        # Читаем файл
        df = pd.read_csv(file.file, encoding=encoding, sep=sep)
    except UnicodeDecodeError:
        raise HTTPException(status_code=400, detail=f"Ошибка декодирования с кодировкой '{encoding}'.")
    ...
    # Ничего не возвращаем, если датасет пустой
    if df.empty:
        return []

    # Берём первые n строк и преобразуем в список словарей
    head_df = df.head(n)
