...
- **Название проекта:** `UpscaleCompare: A Comparative Study of Image Super-Resolution Techniques`
- **Автор:** `Петров Александр Юрьевич`
- **Группа:** `ИКБО-40-24`
- **Контакт:** `@SaTr0V`

**Краткое описание:**  
> Проект посвящен построению веб-сервиса апскейла изображений различными способами - от простых математических методов до нейронных сетей.  
> Используются открытые датасеты DIV2K и FFHQ, FLICKR2K упомянут.  
> Результат - веб-сервис, предлагающий пользователю несколько изображений, улучшенных разными моделями.
...
- `report_rus.md` - отчет по проекту на русском.
